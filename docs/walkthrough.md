...
### 8. Yahoo!ショッピングAPI連携 & 手動入力
- **商品情報補完**: スキャンした商品が未登録の場合、自動でYahoo!ショッピングAPI (`ItemSearch`) を検索し、商品名と画像を補完して登録画面へ遷移します。
- **Client ID管理**: `lib/env.dart` (git管理外) に分離し、セキュリティを確保しました。
- **手動入力**: スキャン画面に追加したキーボードボタンから、JANコードを直接入力して検索・登録が可能になりました。
...
### 9. Firestoreキャッシュ & API制限対策 (Phase 2)
Yahoo!ショッピングAPIのレート制限 (30回/分) を回避するため、Firestoreキャッシュを実装しました。

- **`product_cache` コレクション**:
    - JANコードをDocumentIDとして、商品名・画像を保存。
    - `firestore.rules`: 誰でも読み取り可 (`read: if true`)、認証ユーザーのみ作成可。
- **キャッシュロジック**:
    - 検索時、まずFirestoreを確認。存在すればAPIを呼ばずに即座に表示（高速化＆API節約）。
    - API検索成功時、自動的にキャッシュに保存。
    - APIリクエスト時の **429 (Too Many Requests)** エラーハンドリング (自動リトライ) も実装。
- **商品名クリーニング**:
    - Yahoo API特有のノイズ（【中古】、送料無料、[新品] など）を正規表現で自動削除。
    - 「呪術廻戦 五条悟」のようにクリーンな製品名で保存されるよう調整。
- **その他修正**:
    - `ScanResultScreen` にて `Web Services by Yahoo! JAPAN` のクレジット表記を追加。
    - `posts` コレクションへのアクセス権限不足バグを修正。
